...
static void handle_button_if_needed(void)
{
    ...
    static TickType_t last_accepted = 0;   // час останнього ПРИЙНЯТОГО натиску

    // якщо з попереднього прийнятого пройшло менше DEBOUNCE_MS — ігноруємо
    if ((now - last_accepted) < pdMS_TO_TICKS(DEBOUNCE_MS))
        return;
